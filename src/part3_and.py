...
def plot_attractor_2() -> None:
    ...
    idxs_max = np.unravel_index(np.argmax(ss_ampl, axis=None), ss_ampl.shape)    

    ss_ampl[idxs_max[0]][idxs_max[1]] = ss_ampl[idxs_max[0] + 1][idxs_max[1] + 1]
...
if __name__ == '__main__':
    ...
    '''
    NEED FOLDER 'OUT' WITH BLOCK MATRICES STORED IN .TXT    
    '''
    ss_ampl_2 = plot_attractor_2()    
    ss_ampl_1 = np.transpose(np.loadtxt('values_200.txt'))
    ...
    # plot both
    axs[0].pcolormesh(dof1_mat, dof2_mat, np.round(ss_ampl_1, 2), cmap='bwr', edgecolors='face')    
    axs[1].pcolormesh(dof1_mat, dof2_mat, np.round(ss_ampl_2, 2), cmap='bwr', edgecolors='face')

    axs[0].set_xlabel(r'Initial displacement $q_{1}(0)$ [m]', font='Times New Roman')
    axs[1].set_xlabel(r'Initial displacement $q_{1}(0)$ [m]', font='Times New Roman')

    axs[0].set_ylabel(r'Initial displacement $q_{2}(0)$ [m]', font='Times New Roman')
    ...
    fig.tight_layout()
    plt.savefig('basins_attraction.pdf', dpi=100, bbox_inches='tight')
